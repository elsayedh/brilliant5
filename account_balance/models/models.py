...
class AccountChartOfAccountReport(models.AbstractModel):
    ...
    def _get_lines(self, options, line_id=None):
        ...
        for account, periods_results in accounts_results:
            ...
            account_mybalance =columns[-2]['no_format_name'] - columns[-1]['no_format_name']
            # print(account_mybalance)
            columns.append({'name': self.format_value(account_mybalance, blank_if_zero=True), 'class': 'number', 'no_format_name': account_mybalance})
        ...
        # print(totals)
        # Total report line.
        mytotals=[{'name': self.format_value(total), 'class': 'number'} for total in totals]
        mytotalbalance=totals[-2] -totals[-1]
        mytotals.append({'name': self.format_value(mytotalbalance), 'class': 'number'})
        lines.append({
