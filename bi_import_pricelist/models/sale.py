...
class VendorPricelist(models.TransientModel):
    ...
    import_prod_option = fields.Selection([('name', 'Name'),('code', 'Code'),('barcode', 'Barcode')],string='Select Product By',default='code')
    import_prod_variant_option = fields.Selection([('name', 'Name'),('code', 'Code'),('barcode', 'Barcode')],string='Select Product Variant BY',default='code')
    ...
    def make_pricelist(self, values):
        ...
        if values.get('company'):
            if not company_obj:
                raise Warning(_(' "%s" Company is not available.') % values.get('company'))
...
class SalePricelistWizard(models.TransientModel):
    ...
    def make_pricelist(self, values):
        ...
        if self.company_id.name:
            if not company_obj:
                raise Warning(_(' "%s" Company is not available.') % self.company_id.name)
